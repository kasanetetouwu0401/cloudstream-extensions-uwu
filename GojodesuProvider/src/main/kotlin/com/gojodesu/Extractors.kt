    ...
    private suspend fun parseApi(
        ...
        mirrors.forEach { mirror ->
            ...
            if (resolutions.isEmpty()) {
                resolveMirror("$mainUrl/mirror/$server/$fileId", "$mainUrl/file/$fileId", null, subtitleCallback, callback)
            } else {
                resolutions.forEach { res ->
                    resolveMirror("$mainUrl/mirror/$server/$fileId/$res", "$mainUrl/file/$fileId", res, subtitleCallback, callback)
                }
            }
        }
    }
...
open class EmturbovidExtractor : ExtractorApi() {
    ...
    override suspend fun getUrl(url: String, referer: String?): List<ExtractorLink>? {
        ...
        val page = runCatching { app.get(url, referer = pageRef, headers = headers) }.getOrNull() ?: return null
